...
struct EventsGallery: View {
    ...
    private let events: [AppEventDetail] = [
        .init(
            ...
            imageCandidates: ["therapy", "therapy.webp"],
...
struct VideoCaptureButton: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showRecorder) { VideoRecorderSheet() }
    }
}
        ...
        private func saveVideo(url: URL) {
            ...
            PHPhotoLibrary.requestAuthorization { status in
                ...
                PHPhotoLibrary.shared().performChanges({
                    ...
                    if let error = error { print("Photo save error: \(error)") }
                }
            }
            // Save a copy to app's Documents
            do {
                let fm = FileManager.default
                let docs = fm.urls(for: .documentDirectory, in: .userDomainMask).first!
                let df = DateFormatter(); df.dateFormat = "yyyyMMdd-HHmmss"
                let filename = "evidence-\(df.string(from: Date())).mov"
                let dest = docs.appendingPathComponent(filename)
                try? fm.removeItem(at: dest)
                try fm.copyItem(at: url, to: dest)
                print("Saved video to: \(dest)")
            } catch {
